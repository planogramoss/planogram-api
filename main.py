...
Planogram Scan API
===================
When a person scans a product's UPC/PDI number on their phone:
  1) Checks whether the product is registered as active in the current
     planogram,
  2) Finds its correct shelf/peg location,
  3) If a location (location_id) is also provided during the scan,
     compares what should ACTUALLY be at that location with what was
     scanned,
  4) If there's a mismatch: returns both what should be at that location
     and where the scanned product's real correct location is,
  5) In full shelf-audit mode, compares every scan on the shelf against
     the planogram and reports MISSING / MISPLACED / OK statuses plus
     possible "swap" detection - i.e. if a product was picked up from
     one spot and placed in another, the displaced product also shows
     up in the report.
...
# ---------------------------------------------------------------------------
# The username/password are read from environment variables, NOT hardcoded
# here, because this repository is public on GitHub - anyone could read a
# hardcoded password directly from the source code. Set APP_USERNAME and
# APP_PASSWORD in Render's "Environment" tab (Dashboard > your service >
# Environment > Add Environment Variable). If they are not set, the
# fallback defaults below are used (change these before going live!).
...
def verify_credentials(credentials: HTTPBasicCredentials = Depends(security)):
    # Username is compared case-insensitively (mobile keyboards often
    # auto-capitalize the first letter of a text field, which would
    # otherwise cause a false "wrong password" on phones). The password
    # remains case-sensitive as normal.
    correct_username = secrets.compare_digest(
        credentials.username.strip().lower(), APP_USERNAME.strip().lower()
    )
...
app = FastAPI(
    ...
    description="Planogram location verification and shelf-audit service via UPC/PDI scanning",
    version="1.0.0",
...
def normalize_code(code: str) -> str:
    """Normalization for UPC/PDI comparison: trim whitespace, strip
    leading zeros (so a 12-digit GTIN and a 10-digit PDI code can be
    matched more easily)."""
    if code is None:
        return ""
...
def get_current_slots_flat(db) -> list:
    """Returns a flat list of the current active planogram (for diffing)."""
    rows = db.execute(
...
def reseed_from_slots(db, new_slots: list, meta: dict):
    """Completely replaces the database with a new location list.
    Used when a new PDF is approved. Old scan_log / audit_sessions
    history is preserved - only the products / locations / planogram_slots
    tables are replaced with the new data."""
...
def find_product(db, upc_raw: str):
    """Urun eslestirme, iki asamali:
    1) TAM eslesme (normallestirilmis kod birebir ayni).
    2) ON-EK (prefix) eslesmesi: planogramdaki PDI kodu genelde gercek
       12 haneli UPC-A barkodunun bastaki sifiri atilmis ve sondaki
       kontrol basamagi (check digit) cikarilmis halidir. Yani PDI kodu
       "4400000680" ise, gercek paket barkodu "044000006809" gibi 1-2
       hane daha uzun olabilir ve PDI kodu onun basiyla birebir eslesir.
       Bu yontem yalnizca TEK bir aday urun bulunursa uygulanir - birden
       fazla urun ayni onekle eslesirse (belirsizlik varsa) HICBIRINI
       secmeyiz, yanlis urunle eslestirmek hic eslestirmemekten kotudur."""
    ...
    for row in db.execute("SELECT * FROM products").fetchall():
        pnorm = row["upc_norm"] or ""
        if not pnorm:
            continue
        shorter, longer = (pnorm, norm) if len(pnorm) <= len(norm) else (norm, pnorm)
...
def get_current_pdf():
    ...
    content_disposition_type='inline' ile tarayicida DOGRUDAN goruntulenir
    (indirme kutusu acilmaz), boylece iOS Safari'nin kendi PDF
    goruntuleyicisi devreye girer ve pinch-zoom otomatik calisir."""
